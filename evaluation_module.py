...
def cal_sr(srs_rets, term = 'm'):
    '''Sharpe Ratio 산출; 단위수익률 시리즈, 수집주기'''
    if term == 'm':
...
def get_report2(df_rets, term = 'm'):
    ...
    return pd.concat([get_report(df_rets[col]) for col in df_rets.columns]).set_index(df_rets.columns)
